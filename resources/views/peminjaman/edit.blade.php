...
<div class="max-w-6xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Transaksi Peminjaman</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 bg-blue-600 text-white">
            <h5 class="text-lg font-semibold">Formulir Edit Peminjaman</h5>
        </div>
            ...
            <form action="{{ route('peminjaman.update', $peminjaman) }}" method="POST" id="formPeminjaman">
                        ...
                        <div>
                            ...
                            <select id="mobil_id" name="mobil_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                ...
                                @foreach($mobils as $mobil)
                                    <option value="{{ $mobil->id }}" data-harga="{{ $mobil->harga_per_hari }}" {{ $peminjaman->mobil_id == $mobil->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="supir_id" name="supir_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                ...
                                @foreach($supirs as $supir)
                                    <option value="{{ $supir->id }}" data-harga="{{ $supir->tarif_per_hari }}" {{ $peminjaman->supir_id == $supir->id ? 'selected' : '' }}>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" id="tanggal_pinjam" name="tanggal_pinjam" value="{{ $peminjaman->tanggal_pinjam }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                            </div>

                            <div>
                                <label for="lama_sewa" class="block mb-2 text-sm font-medium text-gray-900">Lama (Hari) <span class="text-red-500">*</span></label>
                                <input type="number" id="lama_sewa" name="lama_sewa" value="{{ $peminjaman->lama_sewa }}" min="1" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                            </div>
                        </div>

                        <div>
                            <label for="tanggal_kembali_rencana" class="block mb-2 text-sm font-medium text-gray-900">Rencana Kembali</label>
                            <input type="date" id="tanggal_kembali_rencana" name="tanggal_kembali_rencana" value="{{ $peminjaman->tanggal_kembali_rencana }}" class="bg-gray-100 border border-gray-300 text-gray-500 text-sm rounded-lg block w-full p-2.5 cursor-not-allowed" readonly>
                        </div>

                        <div class="bg-blue-50 rounded-xl p-6 border border-blue-100">
                            <label class="block mb-2 text-sm font-bold text-blue-800 uppercase tracking-wide">Total Biaya</label>
                            <div class="relative mt-2">
                                <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                                    <span class="text-blue-600 font-bold text-lg">Rp</span>
                                </div>
                                <input type="number" id="harga_total" name="harga_total" value="{{ $peminjaman->harga_total }}" class="bg-white border-2 border-blue-200 text-blue-700 text-2xl font-bold rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-12 p-3 shadow-inner" readonly required>
                            </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                ...
                                <option value="menunggu_persetujuan_pengembalian" {{ $peminjaman->status == 'menunggu_persetujuan_pengembalian' ? 'selected' : '' }}>Menunggu Persetujuan Pengembalian</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end mt-8 pt-6 border-t border-gray-100">
                    <button type="submit" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-lg px-8 py-3 flex items-center gap-2 shadow-lg hover:shadow-xl transition duration-200">
                        <i class="fas fa-check-circle"></i> Update Peminjaman
                    </button>
                </div>
            </form>
        </div>
